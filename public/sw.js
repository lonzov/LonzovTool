  ...
  if (event.data?.type === 'GET_POPUP_DATA') {
    event.ports[0]?.postMessage({
      ...
      popupData: {
        ...
        content: `
        <h4>👾 更新日志：</h4>
        <p>[+] 选择器参数编辑器增加坐标自动计算功能<br>[~] 优化设置页布局和样式<br><b>[+] 新工具：选择器参数编辑器</b><br>[+] PWA安装增加安装包备选方案<br>[~] 修复标签页栏滑动时基点异常导致乱飘<br>[+] 本站支持作为PWA应用安装到本地<br>[+] 新卡片：种子查找器、dxdydz计算、CommandLab、网易敏感词检测</p>
